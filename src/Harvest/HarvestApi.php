...
class HarvestApi {
   ...
   * @return Result
   */
  public function deleteEntry(DayEntry $entry) {
  ...
  public function updateEntry(DayEntry $entry, $other_user = TRUE) {
    ...
    return $this->performPatch($url, $entry->toArray());
  }
  ...
  /**
   * get all project entries for given time range for a particular user if
   * specified
   *
   * <code>
  ...
  /**
   * get all user entries for given time range and for a particular project if
   * specified
   *
   * <code>
   * $range = new Range("20090712", "20090719");
   ...
   * @param  mixed $multi Flag to specify if multiple items are returned by
   *   request
   *
   * @return Result
   */
   ...
   * @return Result
   */
  protected function performPatch($url, $data, $multi = "id") {
    $rData = NULL;
    $code = NULL;
    $success = FALSE;
    while (!$success) {
      $ch = $this->generatePatchCurl($url, $data);
      $rData = curl_exec($ch);
      $code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
